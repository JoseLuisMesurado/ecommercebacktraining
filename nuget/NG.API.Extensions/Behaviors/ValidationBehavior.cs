...
namespace NG.API.Extensions.Behaviors
{
    public class ValidationBehavior<TRequest, TResponse> : IPipelineBehavior<TRequest, TResponse> where TRequest : IRequest<TResponse>
    {
        ...
        public async Task<TResponse> Handle(TRequest request, RequestHandlerDelegate<TResponse> next, CancellationToken cancellationToke)
        {
            ...
            Dictionary<string, string[]> errorsDictionary = (from x in _validators.Select((IValidator<TRequest> x) => x.Validate(context)).SelectMany((FluentValidation.Results.ValidationResult x) => x.Errors)
                                                             where x != null
